          ...
          <div className="w-full rounded-[2.4rem] border border-border bg-surface/95 p-5 sm:p-8">
            ...
            <div className="mt-7 flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
              <div className="text-sm text-muted">
                <p><span className="text-ink">Object</span> <HashText value={wallet.objectId} chars={7} /></p>
                <p className="mt-2">Last digest <HashText value={latest.digest ?? "pending"} chars={7} /></p>
              </div>
              <Link href="/iwallets/demo" data-hover-trigger className="inline-flex items-center justify-center gap-2 rounded-full bg-accent px-8 py-4 text-center text-sm font-semibold text-on-accent hover:bg-accent-soft">
  ...
  return (
    <div className="w-full rounded-[1.8rem] border border-border p-5 lg:flex-1">
      ...
      <div className="mt-8 flex items-end justify-between gap-4">
        ...
        <p className="text-right text-5xl font-light tracking-[-0.06em] text-ink sm:text-6xl">
          {value}<span className="text-dim">.{suffix === "SUI" ? "00" : ""}</span>
        </p>
      </div>
      <p className="mt-3 text-right text-sm text-muted">{sub}</p>
    </div>
  );
}
