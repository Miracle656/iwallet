  ...
  return (
    <section className="rounded-[1.8rem] border border-border bg-surface p-6">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
        <div>
          <p className="inline-flex items-center gap-2 text-sm font-medium text-muted"><HiOutlineLockClosed className="text-accent" />Spending rules</p>
          <h2 className="mt-2 text-2xl font-semibold text-ink">Policy controls for this iWallet</h2>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-muted">Policy controls are secondary in the MVP, but they remain attached to the iWallet rather than the agent.</p>
        </div>
        <span className="inline-flex items-center gap-1.5 rounded-full border border-accent/35 bg-accent/10 px-3 py-1.5 text-xs font-medium text-accent"><HiOutlineShieldCheck />Move enforced</span>
      </div>
...
function Rule({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-[1.25rem] border border-border p-4 md:flex-1">
