  ...
  async function onCreate() {
    ...
    try {
      ...
      if (/insufficient.*gas|GasBalance|no.*gas/i.test(msg)) {
        setError(
          `Sponsor key has no testnet SUI — top up the backend SPONSOR_PRIVATE_KEY address and retry.`,
        );
      } else {
        setError(msg);
      }
            ...
            {ownerPicked ? (
              ...
              >
                ...
                <span className="text-muted text-xs">
                  {zkAddress ? (
                    <HashText value={zkAddress} chars={6} />
                  ) : (
            ...
            {!createdId ? (
              <>
                <div className="border-accent/20 bg-accent/5 text-accent rounded-[1.25rem] border p-4 text-sm">
                  Approve with your Google account to create the iWallet
                  on-chain. Gas is sponsored — no SUI needed.
                </div>
                <button
                  onClick={onCreate}
