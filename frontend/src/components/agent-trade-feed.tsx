  ...
  if (!backendConfigured()) {
    return (
      <div className="rounded-[1.6rem] border border-orange-300/25 bg-orange-300/5 p-5 text-sm text-orange-200">
        Live feed not configured. Set <code className="text-[#e5eef1]">NEXT_PUBLIC_BACKEND_URL</code>{" "}
...
function TradeRow({ t, showOwner }: { t: Trade; showOwner: boolean }) {
  ...
  return (
    <li className="rounded-[1.4rem] border border-white/10 bg-[#101113] p-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-medium text-[#e5eef1]">
              {t.side.toUpperCase()} {t.quantity} on {t.pool}
              <span className="text-[#6f747a]"> @ {t.price}</span>
            </p>
            <p className="mt-0.5 inline-flex items-center gap-2 text-xs text-[#6f747a]">
        ...
        <div className="flex flex-col items-end gap-1 text-xs">
          ...
          {t.withdrawDigest && (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="font-mono text-[#92979d] hover:text-[#e5eef1] hover:underline"
            >
      ...
      {(t.rationale || t.reason || showOwner || t.memoriesUsed) && (
        <div className="mt-3 border-t border-white/10 pt-3">
          ...
          {t.rationale && <p className="text-xs text-[#92979d]">{t.rationale}</p>}
          <div className="mt-2 flex flex-wrap items-center gap-3 text-[11px] text-[#6f747a]">
            {showOwner && (
