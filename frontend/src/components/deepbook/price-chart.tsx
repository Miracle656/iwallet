 ...
 */
export function PriceChart({ pool, pollMs = 5000 }: { pool: Pool; pollMs?: number }) {
  ...
  // poll mid price and append (paused while the tab is hidden)
  usePoll(
    () => {
      if (typeof document !== "undefined" && document.hidden) return;
      fetchMidPrice(pool.key).then((m) => {
        if (m == null) return;
        setMid(m);
        ...
        if (last && last.time === time) {
          ...
          // cap history so a long-open tab doesn't grow memory unbounded
